...
struct binder_state//是表示驱动状态的一个数据结构
{
    int fd;//打开驱动的句柄即文件描述符
    void *mapped;//分配的内存空间
    size_t mapsize;//内存空间的大小
};

/**
 *binder_open
 *主要功能：打开binder设备（/dev/binder），然后将该文件映射到内存中，并返回这块内存的首地
 *址。这样我们就可以像操作内存一样，来操作这个文件了。
*/
struct binder_state *binder_open(const char* driver, size_t mapsize)
{
    struct binder_state *bs;
    struct binder_version vers;
//知识点儿  void* 表示未确定类型的指针。void* 类型可以强制转换为任何其它类型的指针。
    bs = malloc(sizeof(*bs));//向系统申请分配指定size个字节的内存空间。返回类型是 void* 类型
    if (!bs) {
        errno = ENOMEM;
        return NULL;
    }

    bs->fd = open(driver, O_RDWR | O_CLOEXEC);//打开/dev/binder设备节点，返回一个文件描述符，这个描述符很重要，在后面的通讯中会频繁用到
    if (bs->fd < 0) {
    ...
    bs->mapsize = mapsize;
    /**
     将一个文件或者其它对象映射进内存，NULL表示映射区的开始地址系统自动选定地址；mapsize 表示映射区的长度；
     PROT_READ为期望的内存保护标志，表示页内容可以被读取；MAP_PRIVATE 为指定映射对象的类型；
     bs->fd表示有效的文件描述符；0表示被映射对象内容的起点。
    */
